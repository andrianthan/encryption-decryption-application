import os
import pytest
from pathlib import Path

from src.crypto.aes_ccm import (
    generate_key,
    encrypt_file,
    decrypt_file,
)

# test encryption and decryption
def test_ccm_encrypt_decrypt(tmp_path):
    key = generate_key(256)

    infile = tmp_path / "input.txt"
    infile.write_text("hello aes-ccm test")

    outfile = tmp_path / "input.txt.enc"
    decrypted = tmp_path / "output.txt"

    encrypt_file(str(infile), str(outfile), key)
    decrypt_file(str(outfile), str(decrypted), key)

    assert decrypted.read_text() == "hello aes-ccm test"

# test wrong key fails
def test_ccm_wrong_key_fails(tmp_path):
    key1 = generate_key(256)
    key2 = generate_key(256)

    infile = tmp_path / "input.txt"
    infile.write_text("wrong key test")

    outfile = tmp_path / "cipher.enc"
    encrypt_file(str(infile), str(outfile), key1)

    decrypted = tmp_path / "output_bad.txt"

    with pytest.raises(Exception):
        decrypt_file(str(outfile), str(decrypted), key2)

# test tampered ciphertext fails
def test_ccm_tampered_ciphertext_fails(tmp_path):
    key = generate_key(256)

    infile = tmp_path / "input.txt"
    infile.write_text("tampered ciphertext")

    outfile = tmp_path / "cipher.enc"
    encrypt_file(str(infile), str(outfile), key)

    blob = Path(outfile).read_bytes()
    tampered = bytearray(blob)
    # flip bit to corrupt
    tampered[-5] ^= 0xFF 
    Path(outfile).write_bytes(bytes(tampered))

    decrypted = tmp_path / "output.txt"

    with pytest.raises(Exception):
        decrypt_file(str(outfile), str(decrypted), key)

# test tampered header fails
def test_ccm_tampered_header_fails(tmp_path):
    key = generate_key(256)

    infile = tmp_path / "input.txt"
    infile.write_text("tampered header test")

    outfile = tmp_path / "cipher.enc"
    encrypt_file(str(infile), str(outfile), key)

    blob = Path(outfile).read_bytes()
    tampered = bytearray(blob)
    # modify bit to corrupt
    tampered[0] ^= 0x10  
    Path(outfile).write_bytes(bytes(tampered))

    decrypted = tmp_path / "output.txt"

    with pytest.raises(Exception):
        decrypt_file(str(outfile), str(decrypted), key)

# test extra aad
def test_ccm_extra_aad_integrity(tmp_path):
    key = generate_key(256)

    infile = tmp_path / "aad_test.txt"
    infile.write_text("AAD message")

    outfile = tmp_path / "cipher.enc"
    decrypted = tmp_path / "output.txt"

    aad = b"userdata=admin"

    encrypt_file(str(infile), str(outfile), key, extra_aad=aad)

    blob = Path(outfile).read_bytes()
    tampered = bytearray(blob)
    tampered[70] ^= 0x55  
    Path(outfile).write_bytes(bytes(tampered))

    with pytest.raises(Exception):
        decrypt_file(str(outfile), str(decrypted), key)